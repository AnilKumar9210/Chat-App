...
const LeftSec = () => {
  ...
  const {userData,chatData,chatUser,setChatUser,messageId,setMessageId} = useContext (Appcontext);
    ...
    try {
      ...
      if (input.length > 0) {
        ...
        if (!querySnap.empty && querySnap.docs[0].data().id != userData.id) {
          ...
          console.log(userExist?"Hello user exist":"Hello user does not exist")
  ...
  user ? (
    <div className="friend add-friend" onClick={addChat} key={user.id}>
      ...
      <div className="delete" ref={optsRef}>Delete</div>
    </div>
  ) : (
  ...
  <div className="friends-list">
    {chatData?.map((item, index) => (
      <div className="friend" key={index} onClick={()=> setChat(item)}>
        <div className="friend-info" onClick={() => console.log(item)}>
          <img src="https://cdn-icons-png.flaticon.com/512/149/149071.png" alt="x" style={{ height: 40 }} />
